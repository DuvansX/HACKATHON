import { auth, onAuthStateChanged, cerrarSesion, obtenerUrlDeApp } from "../../config/supabase.js";
import {
    crearChat as crearChatEnSupabase,
    eliminarChat as eliminarChatEnSupabase,
    escucharChats as escucharChatsEnSupabase,
    escucharMensajes as escucharMensajesEnSupabase,
    guardarDatosUsuario,
    guardarMensaje,
    migrarChatsLocales,
    renombrarChat as renombrarChatEnSupabase,
    actualizarEstadoChat as actualizarEstadoChatEnSupabase
} from "../shared/supabase-chat-store.js";

const isLocalPreview = window.location.protocol === "file:"
    || ["localhost", "127.0.0.1"].includes(window.location.hostname);

function setupPageLoader() {
    const navigationDirection = sessionStorage.getItem("ideapro-navigation-direction");
    const isEnteringAgent = navigationDirection === "to-chat";

    function setAgentLoadingLabel(loader, visible) {
        const content = loader.querySelector(".page-loader-content");
        if (!content) return;

        let label = content.querySelector(".page-loader-label");
        if (visible && !label) {
            label = document.createElement("p");
            label.className = "page-loader-label";
            label.textContent = "Cargando agente…";
    ...
    function getLoader() {
        let loader = document.getElementById("page-loader") || document.querySelector(".page-loader");
        ...
        loader.innerHTML = '<div class="page-loader-content"><div class="page-loader-grid" aria-hidden="true"><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span><span></span></div></div>';
    ...
    setAgentLoadingLabel(getLoader(), isEnteringAgent);
    ...
    function animateBrand() {
        ...
        if (isDeleting) {
            ...
            if (characterIndex === 0) {
                ...
                isDeleting = false;
                window.setTimeout(animateBrand, 300);
                return;
            }
            window.setTimeout(animateBrand, 90);
            return;
        }
        ...
        if (characterIndex === words[wordIndex].length) {
            isDeleting = true;
            window.setTimeout(animateBrand, 2500);
            return;
        }
        window.setTimeout(animateBrand, 110);
    }

    window.setTimeout(animateBrand, 1600);
}
...
function redirectUnauthenticatedUser(user) {
    if (!user && !isLocalPreview) {
        window.location.href = obtenerUrlDeApp("components/auth/login.html");
    }
}
...
const MENSAJE_ERROR_SINCRONIZACION = "No se pudo sincronizar con Supabase. Verifica tu conexión e inicia sesión de nuevo.";
...
function escucharMensajesDelChat(chatId) {
    ...
    cancelarEscuchaMensajes = escucharMensajesEnSupabase(chatId, (mensajes) => {
...
async function iniciarSincronizacionChats() {
    ...
    cancelarEscuchaChats = escucharChatsEnSupabase((chatsGuardados) => {
...
async function createChat(message) {
    ...
    const creado = await crearChatEnSupabase(chat);
    if (!creado) {
        chats = chats.filter((item) => item.id !== id);
        activeChatId = null;
        renderChatNav();
        throw new Error("No se pudo crear el chat en Supabase.");
    }
...
async function deleteChat(chatId) {
    ...
    const eliminado = await eliminarChatEnSupabase(chatId);
    if (!eliminado) return;
...
async function updateChatState(chatId, changes) {
    const updated = await actualizarEstadoChatEnSupabase(chatId, changes);
...
function appendMessage(sender, content, saveMessage = true) {
    ...
    const contenidoSeguro = sender === "bot"
        ? normalizarRespuestaIA(content)
        : String(content ?? "");
    mostrarTextoPlano(message, contenidoSeguro);
    ...
    if (sender === "bot") {
        ...
        followUpActions.forEach(([label, prompt], index) => {
            const button = document.createElement("button");
            button.type = "button";
            button.className = `bot-follow-up-action ${index === 0 ? "is-secondary" : "is-primary"}`;
    ...
    if (saveMessage) {
        ...
        activeChat.messages.push({ sender, content: contenidoSeguro });
    }
}
...
function normalizarRespuestaIA(content) {
...
function mostrarTextoPlano(element, content) {
    const lineas = String(content ?? "").split("\n");

    lineas.forEach((linea, indice) => {
        element.appendChild(document.createTextNode(linea));
        if (indice < lineas.length - 1) element.appendChild(document.createElement("br"));
    });
}

function obtenerNombreVerificadoGoogle() {
    const nombre = String(auth.currentUser?.displayName ?? "")
        .trim()
        .replace(/\s+/g, " ")
        .replace(/[<>]/g, "")
        .slice(0, 80);
    ...
    const contieneTerminoNoPermitido = /\b(?:puta|puto|mierda|pene|vagina|porno|porn|sexo|follar|cul[oó]|verga|gonorrea)\b/i
        .test(nombreNormalizado);

    return contieneTerminoNoPermitido ? "" : nombre;
}
...
async function sendMessage(message = inputField.value.trim()) {
    ...
    try {
        ...
        console.error("No se pudo iniciar el chat en Supabase.", error);
        mostrarErrorDeSincronizacion();
    ...
    try {
        ...
        const response = await fetch("https://kudos-unskilled-surplus.ngrok-free.dev/webhook/ideapro-chat", {
            method: "POST",
            headers: {
        ...
        const botReply = normalizarRespuestaIA(textoCrudo);
        ...
        const mensajeError = "No fue posible comunicarse con IDEA IA en este momento. Verifica la conexión e inténtalo nuevamente.";
...
chatModalForm?.addEventListener("submit", async (event) => {
    ...
    if (action === "rename") {
        ...
        const actualizado = await renombrarChatEnSupabase(chat.id, tituloActualizado);
...
onAuthStateChanged((user) => {
    ...
    if (user) {
        ...
        if (usuarioSincronizadoId === user.uid) return;
        usuarioSincronizadoId = user.uid;
...
document.querySelectorAll(".back-link, .chat-brand").forEach((link) => {
    link.addEventListener("click", (event) => {
        event.preventDefault();
        sessionStorage.setItem("ideapro-navigation-direction", "to-index");
        document.querySelector(".page-loader-label")?.remove();
        window.ideaproPageLoader?.show();
        document.querySelector(".page-loader")?.classList.add("is-returning");
