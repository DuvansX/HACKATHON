...
function startDiagnosis() {
    const destination = auth.currentUser
        ? obtenerUrlDeApp("components/chat/chat.html")
        : obtenerUrlDeApp("components/auth/login.html");
...
function setupAccessibilityMenu() {
    ...
    const zoomControls = document.querySelectorAll("[data-accessibility-zoom]");
    const quickOptions = document.querySelectorAll("[data-a11y-quick]");

    if (!toggle || !menu || !value || !zoomControls.length) return;

    let zoom = 100;
    const setZoom = (nextZoom) => {
        zoom = Math.min(125, Math.max(80, nextZoom));
        // Escala el texto sin ampliar el lienzo completo, evitando que en móvil
        // se corte el contenido por un desbordamiento horizontal.
        document.documentElement.style.removeProperty("--page-zoom");
        document.documentElement.style.setProperty("--a11y-scale", String(zoom / 100));
    ...
    const setMenuState = (isOpen) => {
        ...
        toggle.setAttribute(
            "aria-label",
            isOpen ? "Cerrar herramientas de accesibilidad" : "Abrir herramientas de accesibilidad"
        );
    };
    ...
    document.addEventListener("click", (event) => {
        if (!event.target.closest(".page-accessibility")) setMenuState(false);
    });
    ...
    zoomControls.forEach((control) => control.addEventListener("click", () => {
    ...
    if (window.matchMedia("(pointer: fine)").matches) {
        ...
        const scheduleClose = () => { leaveTimer = window.setTimeout(() => setMenuState(false), 230); };
